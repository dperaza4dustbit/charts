...
def set_package_digest(chart_entry):
    ...
    pkg_digest = ""
    if "charts.openshift.io/digest" in annotations:
        digest = annotations["charts.openshift.io/digest"]
    ...
    if not pkg_digest and target_digest:
        ...
        if pkg_digest != target_digest:
            raise Exception("Found an integrity issue. SHA256 digest passed does not match SHA256 digest computes")
        chart_entry["digest"] = target_digest
